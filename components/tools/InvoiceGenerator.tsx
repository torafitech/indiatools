  ...
  return (
    <div>
      ...
      <input
        ...
        className="w-full border border-[#F0E4D4] rounded-xl px-4 py-2.5 text-sm text-[#0F2447] focus:outline-none focus:ring-2 focus:ring-[#E8500A]/20 focus:border-[#E8500A] bg-white placeholder:text-[#C4A882] transition-colors"
      />
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <textarea
        ...
        onChange={(e) => onChange(e.target.value)}
        rows={rows}
        className="w-full border border-[#F0E4D4] rounded-xl px-4 py-2.5 text-sm text-[#0F2447] focus:outline-none focus:ring-2 focus:ring-[#E8500A]/20 focus:border-[#E8500A] bg-white placeholder:text-[#C4A882] transition-colors resize-none"
      />
    </div>
  );
}
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#F0E4D4] overflow-hidden">
      ...
      <div className="bg-[#0F2447] px-6 py-5 flex items-start justify-between">
        ...
        <div className="w-10 h-10 rounded-xl bg-[#E8500A] flex items-center justify-center flex-shrink-0">
      ...
      <div className="p-6 space-y-5">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-[#FFFCF8] border-b border-[#F0E4D4]">
                <th className="text-left px-3 py-2 font-bold uppercase tracking-wider text-[#7A6048]">Item</th>
                ...
                <th className="text-right px-3 py-2 font-bold uppercase tracking-wider text-[#7A6048]">Amount</th>
              </tr>
            </thead>
            <tbody>
              {items.map((item, i) => (
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-xs text-[#7A6048]">
            <span>Subtotal</span>
            <span className="tabular-nums font-medium text-[#0F2447]">₹{fmtAmt(calc.subtotal)}</span>
          </div>
          {!isInterState ? (
            <>
              <div className="flex justify-between text-xs text-[#7A6048]">
                <span>CGST</span>
                <span className="tabular-nums">₹{fmtAmt(calc.totalCGST)}</span>
              </div>
              <div className="flex justify-between text-xs text-[#7A6048]">
                <span>SGST</span>
                <span className="tabular-nums">₹{fmtAmt(calc.totalSGST)}</span>
              </div>
            </>
          ) : (
            <div className="flex justify-between text-xs text-[#7A6048]">
              <span>IGST</span>
              <span className="tabular-nums">₹{fmtAmt(calc.totalIGST)}</span>
            </div>
          )}
          <div className="flex justify-between items-center bg-[#0F2447] rounded-xl px-4 py-3">
            <span className="text-sm font-bold text-white">Grand Total</span>
            <span className="text-xl font-bold text-white tabular-nums">₹{fmtAmt(calc.grandTotal)}</span>
          </div>
        </div>
...
export function InvoiceGenerator() {
  ...
  const inputCls = "w-full border border-[#F0E4D4] rounded-xl px-2.5 py-1.5 text-sm text-[#0F2447] focus:outline-none focus:ring-2 focus:ring-[#E8500A]/20 focus:border-[#E8500A] bg-white placeholder:text-[#C4A882] transition-colors";

  return (
    <div className="space-y-5">

      {/* ── Top: Seller / Buyer / Invoice Details + Live Preview ─────────────── */}
      <div className="lg:grid lg:grid-cols-[1fr_360px] lg:gap-8 lg:items-start space-y-5 lg:space-y-0">
        ...
        <div className="space-y-5">
          ...
          {/* Buyer */}
          <SectionCard title="Buyer / Client Details" accentColor="bg-[#4F7CAC]">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div className="sm:col-span-2 lg:col-span-2">
                ...
                <select
                  ...
                  className="w-full border border-[#F0E4D4] rounded-xl px-4 py-2.5 text-sm text-[#0F2447] focus:outline-none focus:ring-2 focus:ring-[#E8500A]/20 focus:border-[#E8500A] bg-white transition-colors"
                >
      ...
      {/* ── Line Items (full width so table has room for all columns) ──────── */}
      <SectionCard title="Line Items" accentColor="bg-[#E8500A]/80">
        <div className="overflow-x-auto -mx-1">
          <table className="w-full min-w-[560px] text-sm">
            <thead>
              <tr className="border-b border-[#F0E4D4]">
                <th className="pb-2.5 text-left pl-1 text-xs font-bold uppercase tracking-widest text-[#7A6048] w-[32%]">Description</th>
                <th className="pb-2.5 text-left text-xs font-bold uppercase tracking-widest text-[#7A6048] w-[11%]">HSN/SAC</th>
                <th className="pb-2.5 text-right text-xs font-bold uppercase tracking-widest text-[#7A6048] w-[9%]">Qty</th>
                ...
                <th className="pb-2.5 text-right text-xs font-bold uppercase tracking-widest text-[#7A6048] w-[16%]">Amount (₹)</th>
            ...
            <tbody className="divide-y divide-[#F0E4D4]">
              {items.map((item, idx) => (
                <tr key={idx}>
                  ...
                  <td className="py-2.5 pr-2 text-right font-semibold text-[#0F2447] text-sm tabular-nums">
                    {fmtAmt(item.qty * item.rate)}
                  </td>
                  <td className="py-2.5">
                    <button
