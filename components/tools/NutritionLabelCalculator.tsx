...
export function NutritionLabelCalculator() {
  ...
  const perServing = useMemo(() => {
    if (totals.totalG === 0) return totals;
    const factor = servingSize / totals.totalG;
    return {
      cal: totals.cal * factor,
        ...
        <div className="flex flex-col sm:flex-row gap-5">
          ...
          <div className="bg-white border-2 border-black rounded-lg p-4 w-full sm:w-72 print:shadow-none" id="nutrition-label">
            ...
            <div className="border-t-4 border-black pt-2 space-y-0.5">
              ...
              {[
                { label: "Total Fat",      val: n(perServing.fat),     unit: "g", dv: 78 },
                { label: "Total Carbohydrate", val: n(perServing.carbs), unit: "g", dv: 300 },
                { label: "Dietary Fiber",  val: n(perServing.fiber),   unit: "g", dv: 28 },
                { label: "Total Protein",  val: n(perServing.protein), unit: "g", dv: 50 },
                { label: "Sodium",         val: Math.round(perServing.sodium), unit: "mg", dv: 2300 },
              ].map((row) => (
